...
// Element-wise ReLU activation for the Eigen matrices/vectors
...
Eigen::VectorXd forward_pass(
    ...
    // Layer 1: Z1 = X * W1 + b1 (X is a 1x784 row vector)
    // The Eigen library handles matrix-vector products as expected.
    // .transpose() turns the (784,) input vector into a (1x784) row vector for multiplication.
    // The result z1 is a (1x128) row vector.
    Eigen::RowVectorXd z1 = input.transpose() * w1;
    z1 += b1.transpose(); // Add the bias vector
    ...
    logits += b2.transpose(); // Add the bias vector
